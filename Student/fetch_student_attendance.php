...
if (!$student_id) { 
    echo "<div class='text-danger'>Not authorized</div>"; 
    exit; 
}
...
if ($class_id <= 0) { 
    echo "<div class='text-danger'>Invalid class ID</div>"; 
    exit; 
}

// --- Class info ---
$classInfo = mysqli_fetch_assoc(mysqli_query(
    $conn, "SELECT section_id, academic_year FROM assign_student WHERE asid=$class_id LIMIT 1"
));
if (!$classInfo) { 
    echo "<div class='text-danger'>Class not found</div>"; 
    exit; 
}
...
$res = mysqli_query($conn, "
    SELECT u.sid, CONCAT(u.first_name,' ',u.father_name,' ',u.grand_father_name) AS full_name
    FROM assign_student ast
    LEFT JOIN students u ON ast.student_id=u.sid
    WHERE ast.section_id = $section_id 
      AND u.student_id = '$student_id_esc'
    LIMIT 1
");
...
if (mysqli_num_rows($res) > 0) {
    ...
    while ($s = mysqli_fetch_assoc($res)) {
        $sid = mysqli_real_escape_string($conn, $s['sid']); // Keep alphanumeric IDs
        ...
        if ($view == 'weekly') {
            ...
            $cond .= " AND attendance_date BETWEEN '$monday' AND '$sunday'";
        } elseif ($view == 'monthly') {
            $month = date('m');
            $year = date('Y');
...
$result2_query = "SELECT status, COUNT(*) as cnt FROM attendance WHERE $cond GROUP BY status";
$result2 = mysqli_query($conn, $result2_query);
...
$percent = $total > 0 ? round(($stats['Present']/$total)*100,1) : 0;
    ...
    echo "  </tbody>
          ...
          function reloadView() {
              ...
              if (typeof loadAttendanceSheet === 'function' && typeof currentClassId !== 'undefined') {
                  loadAttendanceSheet(currentClassId, sem, view, from, to);
              }
          }

          document.getElementById('semesterSelect').addEventListener('change', reloadView);
          document.getElementById('viewSelect').addEventListener('change', reloadView);
    ...
    echo "<div class='text-danger'>No attendance record found for you in this class.</div>";
}
