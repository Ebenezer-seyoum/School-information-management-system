              ...
              <?php else: ?>
                <?php foreach ($cases as $index => $case): ?>
                  <tr>
                    ...
                    <td>
                      <?php
                        $pid = mysqli_real_escape_string($conn, $case['plaintiff'] ?? '');
                        $did = mysqli_real_escape_string($conn, $case['defendant'] ?? '');
                        $pCount = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) AS total FROM `case` WHERE plaintiff = '$pid'"))['total'] ?? 0;
                        $dCount = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) AS total FROM `case` WHERE defendant = '$did'"))['total'] ?? 0;
                        echo "P: $pCount / D: $dCount";
                      ?>
                    </td>
                  </tr>
                <?php endforeach; ?>
