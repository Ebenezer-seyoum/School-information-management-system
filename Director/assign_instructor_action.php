...
if($academic_year && !empty($instructor_ids)){
    ...
    foreach($instructor_ids as $section_id => $instructor_id){
        ...
        $check_q = mysqli_query($conn, "
            SELECT * FROM assign_instructor 
            WHERE section_id = $section_id AND academic_year = '$academic_year_safe'
        ");
        if(mysqli_num_rows($check_q) > 0){
            ...
            $insert_q = mysqli_query($conn, "
                INSERT INTO assign_instructor (section_id, instructor_id, academic_year) 
                VALUES ($section_id, $instructor_id, '$academic_year_safe')
            ");
