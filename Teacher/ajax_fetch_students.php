...
// fetch students
$students=mysqli_query($conn,"SELECT s.sid,s.first_name,s.father_name,s.student_id
...
echo '<table class="table table-bordered text-center align-middle"><thead><tr><th>#</th><th>Student ID</th><th>Full Name</th><th>Subject</th><th>Mark</th><th>Actions</th></tr></thead><tbody>';

$no=1;
while($s=mysqli_fetch_assoc($students)){
    $sid = (int)$s['sid'];
    $full = $s['first_name'].' '.$s['father_name'];
    ...
    echo "<tr>
        ...
        <td>
            <button type='button' class='btn btn-warning btn-sm edit-mark-btn' data-sid='$sid' data-status='$status'>Edit</button>
        </td>
    </tr>";
    $no++;
}
